...
const Residence = () => {
 ...
 useEffect(() => {
  ...
  if (document.cookie !== null) {
   axios
    .get(`http://localhost:8080/api/residences/${data._id}/user`)
    .then((response) => {
 ...
 const onClick = () => {
  ...
  if (value === 0) {
   axios
    .post(`http://localhost:8080/api/residences/${data._id}/user`)
    .then((res) => {
     updateValue(1);
     console.log(res);
    });
   ...
   axios
    .delete(`http://localhost:8080/api/residences/${data._id}/user`)
    .then((res) => {
     updateValue(0);
    });
  }
